...
app.post("/getProduct", async (req, res) => {
  try {
    ...
    const sqlQueryBase = `
            SELECT D.dep_no AS depo_kodu,D.dep_adi AS depo_adi,ISNULL(T.stok_kodu,'') AS stok_kodu,ISNULL(T.stok_isim,'') AS stok_isim,ISNULL(T.barkod,'') AS barkod,ISNULL(T.birim,'') AS birim,ISNULL(T.fiyat,0) AS fiyat,ISNULL(T.son_degisim_tarihi,'1900-01-01') AS son_degisim_tarihi,ISNULL(T.olusturma_tarihi,'1900-01-01') AS olusturma_tarihi,ISNULL(M.MevcutMiktar,0) AS MevcutMiktar FROM DEPOLAR D LEFT JOIN (SELECT S.sto_kod AS stok_kodu,S.sto_isim AS stok_isim,BT.bar_kodu AS barkod,FD.fid_birim_pntr AS birim,FD.fid_yenifiy_tutar AS fiyat,FD.fid_depo_no AS depo_kodu,FD.fid_lastup_date AS son_degisim_tarihi,FD.fid_tarih AS olusturma_tarihi FROM STOKLAR S JOIN BARKOD_TANIMLARI BT ON BT.bar_stokkodu=S.sto_kod JOIN STOK_FIYAT_DEGISIKLIKLERI FD ON FD.fid_stok_kod=S.sto_kod WHERE BT.bar_kodu IN (SELECT LTRIM(RTRIM(CAST(bar_kodu AS nvarchar(50)))) FROM BARKOD_TANIMLARI WHERE LTRIM(RTRIM(CAST(bar_kodu AS nvarchar(50))))='8681183022632' UNION SELECT LTRIM(RTRIM(CAST(bar_kodu AS nvarchar(50)))) FROM BARKOD_TANIMLARI WHERE LTRIM(RTRIM(CAST(bar_kodu AS nvarchar(50)))) LIKE '%8681183022632%')) AS T ON T.depo_kodu=D.dep_no LEFT JOIN (SELECT H.sth_stok_kod AS stok_kodu,CASE WHEN H.sth_tip IN (0,2) THEN H.sth_giris_depo_no WHEN H.sth_tip=1 THEN H.sth_cikis_depo_no END AS depo_kodu,SUM(CASE WHEN (H.sth_tip=0) OR ((H.sth_tip=2) AND (H.sth_giris_depo_no IS NOT NULL)) THEN H.sth_miktar WHEN (H.sth_tip=1) OR ((H.sth_tip=2) AND (H.sth_cikis_depo_no IS NOT NULL)) THEN -H.sth_miktar ELSE 0 END) AS MevcutMiktar FROM STOK_HAREKETLERI H WITH (NOLOCK) WHERE H.sth_stok_kod IN (SELECT bar_stokkodu FROM BARKOD_TANIMLARI WHERE LTRIM(RTRIM(CAST(bar_kodu AS nvarchar(50))))='8681183022632' UNION SELECT bar_stokkodu FROM BARKOD_TANIMLARI WHERE LTRIM(RTRIM(CAST(bar_kodu AS nvarchar(50)))) LIKE '%8681183022632%') GROUP BY H.sth_stok_kod,CASE WHEN H.sth_tip IN (0,2) THEN H.sth_giris_depo_no WHEN H.sth_tip=1 THEN H.sth_cikis_depo_no END) AS M ON M.stok_kodu=T.stok_kodu AND M.depo_kodu=D.dep_no ORDER BY D.dep_no,T.olusturma_tarihi
            `;
